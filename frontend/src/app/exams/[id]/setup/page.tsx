...
const examId = params.id as string; 
  ...
  const [enrollmentStatus, setEnrollmentStatus] = useState<
    'checking' | 'enrolled' | 'not-enrolled' | 'error'
  >('checking');


const [cameraStatus, setCameraStatus] = useState<
  'idle' | 'checking' | 'granted' | 'denied' | 'unavailable'
>('idle');
...
>('idle');
  ...
  const [roomScanStatus, setRoomScanStatus] = useState<
    'idle' | 'starting' | 'scanning' | 'completed' | 'failed' | 'timeout'
  >('idle');
  ...
  /*
   * ---------------------------------------------------------
   * Cleanup media stream and timers
   * ---------------------------------------------------------
   */
  ...
  const checkFaceEnrollment = useCallback(async () => {
    if (!user ) {
      console.log('No user or candidateId available, skipping enrollment check');
      return;
    }
    try {
      ...
      setErrorMessage(
        error?.message || 'Unable to check face enrollment status.'
      );
    } finally {
  ...
  const requestDevicePermissions = async () => {
    try {
      ...
      if (videoTrack && audioTrack) {
        ...
        setErrorMessage(
          'Both camera and microphone are required to continue.'
        );
      }
    } catch (error: any) {
      ...
      setErrorMessage(
        'Camera and microphone permission is required before starting the exam.'
      );
    }
  };
  useEffect(() => {
  if (
    (currentStep === 'verification' || currentStep === 'room-scan') &&
    videoRef.current &&
  ...
  const verifyFace = async () => {
    ...
    try {
       ...
       * The current backend only expects candidateId,
       * but keeping the captured frame here makes the UI
       * ready for a real face-verification API later.
       */
      ...
      if (
        ...
        setErrorMessage(
          response?.message ||
            `Face verification failed. Attempt ${newRetryCount} of ${MAX_FACE_RETRIES}.`
        );
      }
    } catch (error: any) {
  ...
  const startRoomScan = async () => {
    ...
    try {
      ...
      /*
       * There is currently no /frame endpoint.
       * Therefore this is client-side guided timing.
       */
      ...
      setErrorMessage(
        error?.message || 'Unable to start room scan.'
      );
    }
  };
    ...
    try {
      ...
      if (
        ...
        toast({
          ...
          description: 'Your environment check is complete.',
        });
        ...
        setErrorMessage(
          response?.message || 'Room scan could not be completed.'
        );
      }
    } catch (error: any) {
      console.error('Room scan completion error:', error);

      setRoomScanStatus('failed');

      setErrorMessage(
        error?.message || 'Failed to complete room scan.'
      );
    }
  };
  ...
  const markRoomScanIncomplete = async () => {
    ...
    setRoomScanStatus('failed');
    setErrorMessage(
      'Room scan was not completed. Please try again.'
    );
  };
    ...
    try {
      ...
      if (
        ...
        setCurrentStep('complete');
         ...
         sessionStorage.setItem(`exam-session-${examId}`, sessionId);
         
        toast({
          title: 'Ready to start',
          description: 'All pre-exam checks have passed.',
        });
        ...
        setErrorMessage(
          response?.message ||
            'Unable to start the proctoring session.'
        );
      }
    } catch (error: any) {
      console.error('Proctoring session error:', error);

      setSessionStatus('failed');

      setErrorMessage(
        error?.message ||
          'Failed to start the proctoring session.'
      );
    }
  };
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-600 mt-2">
                Complete all verification steps before entering the exam.
              </p>
            </div>
          ...
          <CardContent className="p-6">
              ...
              <div className="max-w-xl mx-auto text-center py-8">
                ...
                {checkingEnrollment ? (
                  ...
                  <>
                    ...
                    <p className="text-slate-600 mt-2">
                      You need to complete face enrollment before
                      starting this exam.
                    </p>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="border rounded-xl p-5">
                    ...
                    <p className="text-sm mt-4 text-slate-600">
                      Your camera is required for identity
                  ...
                  <div className="border rounded-xl p-5">
                    ...
                    <p className="text-sm mt-4 text-slate-600">
                      Your microphone is required for the
            ...
            {currentStep === 'verification' && (
              <div className="max-w-2xl mx-auto text-center">
                ...
                <div className="mt-5">
                  <p className="text-sm text-slate-500">
                    Verification attempts: {faceRetries}/
                    {MAX_FACE_RETRIES}
                  </p>
                </div>
                ...
                {verificationStatus === 'verifying' ? (
                  ...
                  <Button
                    onClick={verifyFace}
                    disabled={faceRetries >= MAX_FACE_RETRIES}
                    className="mt-5 min-w-[220px]"
                  >
            ...
            {currentStep === 'room-scan' && (
              <div className="max-w-2xl mx-auto text-center">
                ...
                <p className="text-slate-600 mt-2">
                  Slowly move your camera around the room so
                ...
                {roomScanStatus === 'scanning' && (
                  <p className="mt-5 text-sm text-blue-600 font-medium">
                    Slowly pan your camera around the room...
                  </p>
                )}

                {roomScanStatus === 'completed' && (
                  <div className="mt-6 text-green-600 font-semibold flex justify-center gap-2">
                    <CheckCircle2 />
                    Room scan completed
                  </div>
                )}
                ...
                {roomScanStatus === 'scanning' && (
                  <Button
                    ...
                    onClick={markRoomScanIncomplete}
                  >
                    Stop Room Scan
                  </Button>
                )}
            ...
            {currentStep === 'complete' && (
              <div className="max-w-xl mx-auto text-center py-8">
                ...
                <p className="text-slate-600 mt-3">
                  Your identity, device and room verification
